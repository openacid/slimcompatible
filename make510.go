...
func main() {
	...
	for typ, ks := range testkeys.Keys {
		...
		prf := "slimtrie-data-" + typ + "-nopref-"
		makeMarshaledData(prf+"%s", f.Ver, ks, &trie.Opt{})

		prf = "slimtrie-data-" + typ + "-innpref-"
		makeMarshaledData(prf+"%s", f.Ver, ks, &trie.Opt{
			InnerPrefix: trie.Bool(true)})

		prf = "slimtrie-data-" + typ + "-allpref-"
		makeMarshaledData(prf+"%s", f.Ver, ks, &trie.Opt{
			InnerPrefix: trie.Bool(true),
...
func makeMarshaledData(fn string, defaultVer string, keys []string, opt *trie.Opt) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("failed:", fn, defaultVer, r)
		}
	}()
	makeData(fn, defaultVer, keys, opt)
}

func makeData(fn string, defaultVer string, keys []string, opt *trie.Opt) {
	...
	st, err := trie.NewSlimTrie(I32{}, keys, values, *opt)
	if err != nil {
		panic(err)
	}
